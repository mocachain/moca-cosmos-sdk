...
* (x/staking) [#391](https://github.com/mocachain/moca-cosmos-sdk/pull/391) Add missing `defer iterator.Close()` in `NonNegativePowerInvariant` to prevent an iterator/resource leak when a validator record is missing for a power-store entry. (MOCA-817)
...
* (x/gov) `ChargeDeposit` routed cancellation charges to the community pool only when `ProposalCancelDest` matched the distribution module address's one canonical string rendering; any other validly-decodable spelling of the same address instead fell through to a direct account transfer, which credits the module's balance but not `x/distribution`'s community-pool ledger. Now compares by decoded address value, so every valid spelling of the same address routes through `distrKeeper.FundCommunityPool` consistently. Added a regression test covering a non-canonical `ProposalCancelDest` spelling. (MOCA-1263)
* (x/gov) `CancelProposal` compared the proposal's stored proposer against the caller's address as raw strings, so a proposer submitting `MsgCancelProposal` with a validly-different spelling of their own address than what got stored at submission time was wrongly rejected as an invalid proposer. Now compares by decoded address value. Added a regression test. (MOCA-1263)
* (x/authz) Compare grantee/granter addresses by decoded value rather than string form in `Grant`/`Revoke`. Added a regression test covering a non-canonical self-grant spelling. (MOCA-1263)
* (x/feegrant) Compare grantee/granter addresses by decoded value rather than string form in `GrantAllowance`, `RevokeAllowance`, and `Grant.ValidateBasic`. Added regression test cases for a non-canonical self-grant spelling (currently blocked from running by a pre-existing, unrelated test-infra gap in this module — see PR discussion). (MOCA-1263)
* (x/group) Compare admin and proposer addresses by decoded value rather than string form across group-admin, group-policy-admin, and proposal-withdrawal authorization checks. Added regression tests for `isProposer`, `WithdrawProposal`, `doUpdateGroup`, and `doUpdateGroupPolicy` covering non-canonical address spellings. (MOCA-1263)
* (x/gov, x/authz, x/feegrant, x/group) The above address-comparison fixes change what a given message accepts or how it routes funds for the same on-chain inputs (a previously string-mismatched-but-same-account input now succeeds; `ChargeDeposit` now takes a different code path for some previously-accepted inputs). Nodes on old and new binaries can disagree on the result for identical inputs, so roll these out as a coordinated upgrade rather than an independent point release. (MOCA-1263)
* (x/auth, x/bank, x/consensus, x/crisis, x/distribution, x/gov, x/mint, x/slashing, x/staking, x/upgrade) Governance-gated `UpdateParams`-style handlers (plus `x/gov`'s `ExecLegacyContent`/`UpdateCrossChainParams` and `x/upgrade`'s `SoftwareUpgrade`/`CancelUpgrade`) compared the configured module authority against the message's `Authority` field as raw strings, so a governance proposal whose message spelled the authority address differently than how it's canonically wired (`simapp/app.go`) could pass a vote and then fail at execution with an invalid-signer error. Now compares by decoded address value. This widens what's accepted for the same on-chain inputs, so nodes on old and new binaries can disagree on whether a proposal message executes — roll out as a coordinated upgrade, not an independent point release. Added regression tests through representative handlers (`x/consensus`, `x/crisis`, `x/mint`) covering a valid non-canonical authority spelling, alongside those suites' existing malformed-authority error-shape cases.
* (x/genutil) `add-genesis-account`/`AddGenesisAccounts` looked up an existing genesis balance entry to top up by comparing addresses as raw strings (account-existence itself was already checked correctly by decoded value); a mismatch silently skipped the top-up while still incrementing declared bank supply, so the two could drift apart in a generated genesis file. `collect-gentxs`' account-funded check (`ValidateAccountInGenesis`) had the same raw-string comparison and could wrongly reject a validator's gentx as unfunded. Both now compare by decoded address value. Added regression tests covering a balance stored under a non-canonical spelling for all three paths (single/batch top-up asserting the supply-equals-balances invariant and no duplicate entry, plus the gentx funded-account check).
...
* (x/tx, x/auth/tx) `SIGN_MODE_DIRECT_AUX`'s fee-payer self-check (in both `x/tx/signing/directaux` and its unused duplicate in `x/auth/tx`) compared addresses with a plain string/`EqualFold` match instead of by decoded value. Neither implementation is reachable from this fork's actual signature-verification path today (the wired ante-handler decorator verifies every transaction as EIP-712 regardless of declared sign mode, and the generic sign-mode dispatcher that would reach either of these has no live callers), so this is hardening rather than a live-path fix — worth doing so a future refactor that re-enables generic sign-mode dispatch doesn't inherit the bug silently. Added end-to-end regression coverage through the `x/tx` handler's `GetSignBytes` (fee payer and signer as the same decoded address in differing valid spellings, both explicit and derived fee-payer paths), and repaired the package's existing handler test, which previously failed at setup for lack of a validator address codec.
