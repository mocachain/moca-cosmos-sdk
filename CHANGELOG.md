...
* (x/staking) Add missing `defer iterator.Close()` in `NonNegativePowerInvariant` to prevent an iterator/resource leak when a validator record is missing for a power-store entry. (MOCA-817)
