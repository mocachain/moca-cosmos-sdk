...
// x/gov/keeper: that package's own test binary does not compile on main for
// pre-existing reasons unrelated to the code under test here (bech32-oriented
...
// MOCA-1362 (tally read side, already on main via #282): Tally keys its
// validator-power map by the canonical EIP-55 rendering of each validator's
// address and looks delegations up by the same canonical rendering. Before that
// fix the map was keyed by the raw, as-stored OperatorAddress string, so a
// validator whose operator address was stored in a non-canonical (e.g.
// all-lowercase) spelling was keyed under a string the canonical delegation
// lookup never matched — dropping its entire bonded voting power out of the
// tally with no error. This test guards that read-side behaviour on main; it is
// the counterpart to the same test on the release backport (#302).
//
...
// rendering (as any lookup that re-derives the canonical form produces). It then
// has that validator self-vote Yes and asserts the vote is counted and the
...
func TestTally_NonCanonicalValidatorOperatorAddress(t *testing.T) {
	...
	// main's gov NewKeeper takes no crossChainKeeper arg (unlike release/1.4.x)
	// and a trailing variadic initOptions we omit. bank / distr keepers are never
	// touched by Tally, so nil is safe.
	k := govkeeper.NewKeeper(
		encCfg.Codec, storeService,
		fakeAccountKeeper{}, nil, sk, nil,
...
func (fakeStakingKeeper) ValidatorAddressCodec() addresscodec.Codec { return hexCodec{} }
...
func (fakeAccountKeeper) AddressCodec() addresscodec.Codec { return hexCodec{} }

// hexCodec is a moca-style 0x-hex address codec; it exists only to satisfy the
// AddressCodec()/ValidatorAddressCodec() interface methods and is never invoked
// by Tally or NewKeeper.
