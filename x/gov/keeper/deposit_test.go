		...
		for i := range 3 {
			...
			t.Run(testName(i), func(t *testing.T) {
				...
				if len(params.ProposalCancelDest) != 0 {
					accAddr, err := sdk.AccAddressFromHexUnsafe(params.ProposalCancelDest)
					require.NoError(t, err)
...
// trackMockBalances above: ChargeDeposit only touches Deposits/bankKeeper/
// authKeeper/distrKeeper (not Params or Proposals), and a purpose-built
// gomock expectation (FundCommunityPool exactly once,
...
func TestChargeDeposit_NonCanonicalCommunityPoolDest(t *testing.T) {
	...
	depositAmount := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom, sdkmath.NewInt(1000)))
