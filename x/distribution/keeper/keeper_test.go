...
func TestWithdrawValidatorCommission_BlockedWithdrawAddress(t *testing.T) {
	...
	require.NoError(t, distrKeeper.SetValidatorOutstandingRewards(ctx, sdk.AccAddress(valAddr), types.ValidatorOutstandingRewards{Rewards: valCommission}))
	require.NoError(t, distrKeeper.SetValidatorAccumulatedCommission(ctx, sdk.AccAddress(valAddr), types.ValidatorAccumulatedCommission{Commission: valCommission}))
	...
	_, err := distrKeeper.WithdrawValidatorCommission(sdkCtx, sdk.AccAddress(valAddr))
...
func TestWithdrawDelegationRewards_BlockedWithdrawAddress(t *testing.T) {
	...
	stakingKeeper.EXPECT().Validator(gomock.Any(), sdk.AccAddress(valAddr)).Return(val, nil).AnyTimes()
	stakingKeeper.EXPECT().Delegation(gomock.Any(), addr, sdk.AccAddress(valAddr)).Return(del, nil).AnyTimes()

	require.NoError(t, distrtestutil.CallCreateValidatorHooks(ctx, distrKeeper, addr, sdk.AccAddress(valAddr)))
	...
	_, err = distrKeeper.WithdrawDelegationRewards(sdkCtx, addr, sdk.AccAddress(valAddr))
	require.ErrorIs(t, err, sdkerrors.ErrUnauthorized)
}
