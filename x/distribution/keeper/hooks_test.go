...
func TestBeforeDelegationSharesModified(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			stakingKeeper.EXPECT().Delegation(gomock.Any(), owner, sdk.AccAddress(valAddr)).Return(del, nil).AnyTimes()

			require.NoError(t, distrtestutil.CallCreateValidatorHooks(ctx, distrKeeper, owner, sdk.AccAddress(valAddr)))
			...
			err = distrKeeper.Hooks().BeforeDelegationSharesModified(hookCtx, owner, sdk.AccAddress(valAddr))
			...
			if tc.expectedWithdrawAddress == disttypes.AttributeValueCommunityPool {
				...
				outstanding, err := distrKeeper.GetValidatorOutstandingRewardsCoins(ctx, sdk.AccAddress(valAddr))
				require.NoError(t, err)
...
func TestAfterValidatorRemoved(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = distrKeeper.Hooks().AfterValidatorRemoved(hookCtx, valConsAddr0, sdk.AccAddress(valAddr))
			if tc.expectedError != nil {
