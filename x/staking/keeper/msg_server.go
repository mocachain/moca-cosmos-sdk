...
func (k msgServer) CreateValidator(ctx context.Context, msg *types.MsgCreateValidator) (*types.MsgCreateValidatorResponse, error) {
	...
	// EIP-55 checksummed form re-derived on lookup (e.g. gov tally keys its
	// validator map by the canonical string) — the same canonical form already
	// used for delAddr/relayerAddr/challengerAddr below.
