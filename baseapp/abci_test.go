...
func TestABCI_InitChain(t *testing.T) {
	...
	app.InitChain(abci.RequestInitChain{ChainId: "moca_1000000-1"})
	res := app.Query(query)
	...
	initChainRes := app.InitChain(abci.RequestInitChain{AppStateBytes: []byte("{}"), ChainId: "moca_1000000-1"}) // must have valid JSON genesis file, even if empty
	...
	require.Equal(t, "moca_1000000-1", chainID, "ChainID in deliverState not set correctly in InitChain")

	chainID = getCheckStateCtx(app).ChainID()
	require.Equal(t, "moca_1000000-1", chainID, "ChainID in checkState not set correctly in InitChain")
